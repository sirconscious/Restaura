...
export default function DishCard({ project, showMeals }) {
    return (
        <div className="group rounded-lg shadow-lg overflow-hidden relative">
            <img
                src={project.image}
                alt={project.title}
                className={`w-full ${showMeals ? "h-44" : "h-48"} object-cover rounded-3xl p-2`}
            />
            <div className={`absolute inset-0 flex justify-center items-center ${showMeals ? "" : "bg-black bg-opacity-50 transition-transform duration-500 transform translate-y-full group-hover:translate-y-20"
                }`}>
