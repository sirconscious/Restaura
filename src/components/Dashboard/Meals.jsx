    ...
    const fetchMeals = async () => {
      try {
        const response = await axios.get(
          "http://localhost/dashboard_php/fetchMeals.php",
        );
        setMeals(response.data.meals); // Set all meals regardless of availability
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost/dashboard_php/UpdateMeals.php",
        {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            const selectedMeal = meals.find(
              (meal) => meal.meal_id === parseInt(selected),
            );
        ...
        >
          ...
          {meals.map((meal) => (
            <option
              className={meal.is_available ? "text-green-500" : "text-red-500"}
              key={meal.meal_id}
              value={meal.meal_id}
            >
