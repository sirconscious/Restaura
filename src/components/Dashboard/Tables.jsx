...
const TableUpdate = () => {
  ...
  useEffect(() => {
    ...
    const fetchTables = async () => {
      try {
        const response = await axios.get('http://localhost/bibilo_crud/test3/fetchTables.php');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost/bibilo_crud/test3/UpdateTables.php', {
